...
module Trace :
  sig
    ...
    | [] -> ()
    | (t1, t2)::xs ->
      Term.Flat.marshal chan t1 ;
      Term.Flat.marshal chan t2 ;
      marshal chan xs

    let[@tail_mod_cons] rec unmarshal ?(env=0) ?(scope=Term.Var.non_local_scope) chan =
      match Term.Flat.unmarshal ~env ~scope chan with
      | exception End_of_file -> []
      | t1 ->
        let t2 = Term.Flat.unmarshal ~env ~scope chan in
...
let check_is_var ({env; subst} : State.t) (x : 'a ilogic) : bool =
  match Env.shape_flat env @@ Subst.shallow_apply env subst @@ Term.repr x with
  | Var _ -> true
  | _ -> false
