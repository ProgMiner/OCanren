  ...
  struct
    ...
    let add env t var term =
      try
        ...
        Term.VarMap.add var terms t
      with Not_found ->
        Term.VarMap.add var (S.singleton term) t

    let mem env t var term =
      try S.mem term @@ Term.VarMap.find var t
    ...
    let subsumed env t =
       ...
       *)
      Term.VarMap.for_all begin fun var terms' ->
        try
...
module Disjunct :
  ...
  struct
    ...
    let rec recheck env subst (t: t): t  =
      ...
      match refine env subst (Term.repr var) term with
      | Fulfiled       ->
          raise Disequality_fulfilled
      | Refined delta  -> (
...
module Conjunct :
  sig
    ...
    let reify env subst t x =
      ...
      in
      (*
      Format.printf "SIMPL: %a\n" pp t ;
      *)
      let fv = Subst.freevars env subst @@ Term.repr x in
      (*
      Format.printf "FVS(%a): %a\n" Term.pp (Term.repr x) (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf ", ") (fun ppf x -> Term.pp ppf (Term.repr x))) (Term.VarSet.elements fv) ;
      *)
      let t = project env subst t fv in
      (*
      Format.printf "PROJECT: %a\n" pp t ;
      *)
...
let reify env subst cstore x = Conjunct.reify env subst (combine env subst cstore) x

let reify_t env subst = Term.VarMap.map @@ Conjunct.reify_t env subst

(*
let reify env subst cstore x =
  Format.printf "CSTORE: %a\n" pp cstore ;
  let res = combine env subst cstore in
  Format.printf "COMBINE: %a\n" Conjunct.pp res ;
  let res = Conjunct.reify env subst res x in
  let hlp ppf xs =
    Format.fprintf ppf "{ " ;
    Term.VarMap.iter (fun x y -> Format.fprintf ppf "%a -> %a ; " Term.pp (Term.repr x)
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf ", ") Term.pp) (Answer.S.elements y)) xs ;
    Format.fprintf ppf "}"
  in
  Format.printf "ANSWER: %a\n" (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf ", ") hlp) res ;
  res
*)
