...
module Binding =
  struct
    ...
    let is_relevant env vs { var; term } =
      ...
      Env.unterm env term ~fvar:(fun v -> Term.VarSet.mem v vs) ~fval:(fun _ _ -> false)
        ~fcon:(fun _ _ _ -> false) ~fmu:(fun _ -> false)
    *)
...
(*
(* following functions will hang on recursive terms *)
...
(* [term] must be either a variable or a term head, [var] must be free in [subst] *)
...
(* [term] mustn't be a variable or mu-binder, [var] must be free in [subst]
...
(* TODO(ProgMiner): implement subsume or remove it usages *)
